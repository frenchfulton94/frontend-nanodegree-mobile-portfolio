...
changeSliderLabel(size);function changePizzaSizes(size){var elem=document.querySelectorAll(".randomPizzaContainer");var oldWidth=elem[0].offsetWidth;var windowWidth=document.querySelector("#randomPizzas").offsetWidth;var oldSize=oldWidth/windowWidth;function sizeSwitcher(size){switch(size){case"1":return 0.25;case"2":return 0.3333;case"3":return 0.5;default:console.log("bug in sizeSwitcher");}}
var newSize=sizeSwitcher(size);var dx=(newSize-oldSize)*windowWidth;var newwidth=(elem[0].offsetWidth+dx)+'px';for(var i=0;i<elem.length;i++){elem[i].style.width=newwidth;}}
...
function updatePositions(){frame++;window.performance.mark("mark_start_frame");window.requestAnimationFrame(function(){var items=document.querySelectorAll('.mover');var phase=function(math){return Math.sin(math);};var tops=(document.body.scrollTop/1250);console.log(document.body.scrollTop);var left;for(var i=0;i<items.length;i++){left=items[i].basicLeft;items[i].style.left=left+100*phase(tops+(i%5))+'px';}});window.performance.mark("mark_end_frame");window.performance.measure("measure_frame_duration","mark_start_frame","mark_end_frame");if(frame%10===0){var timesToUpdatePosition=window.performance.getEntriesByName("measure_frame_duration");logAverageFrame(timesToUpdatePosition);}}
window.addEventListener('scroll',updatePositions);document.addEventListener('DOMContentLoaded',function(){var cols=8;var s=256;var elem;for(var i=0;i<40;i++){elem=document.createElement('img');elem.className='mover';elem.src="images/pizza.png";elem.style.height="100px";elem.style.width="73.333px";elem.basicLeft=(i%cols)*s;elem.style.top=(Math.floor(i/cols)*s)+'px';document.querySelector("#movingPizzas1").appendChild(elem);}
updatePositions();});