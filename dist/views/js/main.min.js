...
changeSliderLabel(size);function changePizzaSizes(size){var elem=document.querySelectorAll(".randomPizzaContainer");var oldWidth=elem[0].offsetWidth;var windowWidth=document.querySelector("#randomPizzas").offsetWidth;var oldSize=oldWidth/windowWidth;var newSize;switch(size){case"1":newSize=0.25;break;case"2":newSize=0.3333;break;case"3":newSize=0.5;break;default:console.log("bug in sizeSwitcher");}
...
function updatePositions(){frame++;window.performance.mark("mark_start_frame");window.requestAnimationFrame(function(){var items=document.querySelectorAll('.mover');var tops=(document.body.scrollTop/1250);var phase=[];for(var i=0;i<5;i++){phase.push(Math.sin(tops+i)*100);}
...
window.addEventListener('scroll',updatePositions);document.addEventListener('DOMContentLoaded',function(){var cols=8;var s=256;var elem;var height=window.innerHeight;var numOfPizzas=height/s*cols;for(var i=0;i<numOfPizzas;i++){elem=document.createElement('img');elem.className='mover';elem.src="images/pizza.png";elem.style.height="100px";elem.style.width="73.333px";elem.basicLeft=(i%cols)*s;elem.style.top=(Math.floor(i/cols)*s)+'px';document.querySelector("#movingPizzas1").appendChild(elem);}
updatePositions();});